...
def member_status():
    ...
    if member:
        ...
        if member_has_past_purchases(member):
...
def verify_logout():
    ...
    if verify_login_email and member_email and secrets.compare_digest(member_email, verify_login_email):
...
def legacy_member_invite_signup():
    ...
    if request.method == 'POST':
        ...
        if not token_valid:
            ...
            if ok:
                session['legacy_member_email'] = email
                return redirect('/?open_tickets=1&apply_member_discount=1')
