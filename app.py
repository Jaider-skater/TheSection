...
events_file = os.getenv('EVENTS_FILE', os.path.join(os.path.dirname(__file__), 'data', 'events.json'))
LEGACY_EVENT_ID = os.getenv('LEGACY_EVENT_ID', '2025-10-24')
SCAN_EARLY_HOURS = int(os.getenv('SCAN_EARLY_HOURS', '2'))
SCAN_LATE_HOURS = int(os.getenv('SCAN_LATE_HOURS', '1'))
...
def default_events():
    return [{
        ...
        'date_display': 'Saturday, October 24th',
        'time_display': '10:00 PM – 2:00 AM',
        'venue': 'The Gem, Idaho Falls',
        'starts_at': '2025-10-24T22:00:00-06:00',
        'ends_at': '2025-10-25T02:00:00-06:00',
        'flyer_image': 'images/TheSectionHalloweenFlyer.JPG',
        'sales_open': True,
        'ga_price_cents': int(os.getenv('GA_PRICE_CENTS', '1000')),
        'vip_price_cents': int(os.getenv('VIP_PRICE_CENTS', '2500')),
        'description': 'October 24th • 10PM - 2AM @ The Gem',
    }]
...
def load_events():
    ...
    try:
        ...
        print(f'Failed to load events file ({events_file}):', e)
        return default_events()
...
def save_events(events):
    ...
    try:
        ...
        print(f'Failed to save events file ({events_file}):', e)
        return False
...
def bootstrap_events():
    ...
    with events_lock:
        ...
        print(f'Bootstrap events file created: {events_file}')
...
def get_event(event_id):
    ...
    for event in load_events():
        if event.get('id') == normalized:
            return event
    return None


def ticket_event_id(record):
    if not record:
        return LEGACY_EVENT_ID
    return record.get('event_id') or LEGACY_EVENT_ID


def event_ticket_types(event):
    if not event:
        return TICKET_TYPES
    ga_price = int(event.get('ga_price_cents') or TICKET_TYPES['general']['price_cents'])
    vip_price = int(event.get('vip_price_cents') or TICKET_TYPES['vip']['price_cents'])
    description = event.get('description') or (
        f"{event.get('date_display', event.get('name', 'The Section'))} @ {event.get('venue', 'The Gem')}"
    )
    return {
        'general': {
            'name': 'General Admission',
            'price_cents': ga_price,
            'description': description,
            'access': None,
        },
        'vip': {
            'name': 'VIP Admission',
            'price_cents': vip_price,
            'description': 'Includes upstairs seating + priority entry',
            'access': 'Upstairs seating',
        },
    }
...
def get_ticket_types(event_id=None):
    ...
    if event:
        return event_ticket_types(event)
    return TICKET_TYPES


def event_public_summary(event):
    if not event:
        return None
    types = event_ticket_types(event)
    ga = types['general']['price_cents']
    vip = types['vip']['price_cents']
    return {
        'id': event.get('id'),
        'slug': event.get('slug'),
        'name': event.get('name'),
        'headline': event.get('headline') or event.get('name'),
        'date_display': event.get('date_display'),
        'time_display': event.get('time_display'),
        'venue': event.get('venue'),
        'flyer_image': event.get('flyer_image'),
        'sales_open': bool(event.get('sales_open', True)),
        'ga_price_cents': ga,
        'vip_price_cents': vip,
        'price_label': f"GA ${ga / 100:.0f} · VIP ${vip / 100:.0f}",
...
def list_public_events():
    ...
    events = []
    for event in load_events():
        summary = event_public_summary(event)
        if not summary:
            continue
        end = parse_event_datetime(event.get('ends_at'))
        sales_open = bool(event.get('sales_open', True))
        summary['is_past'] = bool(end and now > end)
        summary['is_upcoming'] = not summary['is_past']
        if sales_open and not summary['is_past']:
            events.append(summary)
    events.sort(key=lambda item: parse_event_datetime(item.get('starts_at')) or now)
    return events


def get_sellable_event(event_id):
    event = get_event(event_id) if event_id else None
    if event:
        ...
        if end and now_utc() > end:
            return None
        return event
    for summary in list_public_events():
        resolved = get_event(summary.get('id'))
        if resolved:
            return resolved
    return None


def event_scan_phase(event, now=None):
    now = now or now_utc()
    start = parse_event_datetime(event.get('starts_at'))
    end = parse_event_datetime(event.get('ends_at'))
    if not start or not end:
        return 'unknown'
    early = start - timedelta(hours=SCAN_EARLY_HOURS)
    late = end + timedelta(hours=SCAN_LATE_HOURS)
    if now > late:
        return 'past'
    if now < early:
        return 'future'
...
def get_scanner_context(now=None):
    now = now or now_utc()
    events = sorted(
        load_events(),
        key=lambda event: parse_event_datetime(event.get('starts_at')) or now,
    )
    live_events = [event for event in events if event_scan_phase(event, now) in ('live', 'early')]
    if live_events:
        target = live_events[0]
        return {
            'mode': 'live',
            'target_event': target,
            'target_event_id': target.get('id'),
            'upcoming_event': None,
            'message': f"Live now — {target.get('name', target.get('id'))}",
            'detail': target.get('date_display') or target.get('time_display') or '',
        }

    upcoming = [event for event in events if event_scan_phase(event, now) == 'future']
    if upcoming:
        nxt = upcoming[0]
        start = parse_event_datetime(nxt.get('starts_at'))
        return {
            'mode': 'upcoming',
            'target_event': None,
            'target_event_id': None,
            'upcoming_event': nxt,
            'message': f"Next up — {nxt.get('name', nxt.get('id'))}",
            'detail': nxt.get('date_display') or '',
            'starts_at': start.isoformat() if start else nxt.get('starts_at'),
        }

    past = [event for event in events if event_scan_phase(event, now) == 'past']
    if past:
        last = past[-1]
        return {
            'mode': 'past',
            ...
            'upcoming_event': None,
            'last_event': last,
            'message': f"Last event ended — {last.get('name', last.get('id'))}",
            'detail': last.get('date_display') or '',
        }
...
def scanner_context_public(ctx):
    if not ctx:
        ctx = get_scanner_context()
    target = ctx.get('target_event')
    upcoming = ctx.get('upcoming_event')
    last = ctx.get('last_event')
    active = target or upcoming or last
    return {
        'mode': ctx.get('mode'),
        'message': ctx.get('message'),
        'detail': ctx.get('detail'),
        'target_event_id': ctx.get('target_event_id'),
        'active_event': event_public_summary(active) if active else None,
    }


def validate_ticket_for_scanner(record):
    ctx = get_scanner_context()
    ticket_eid = ticket_event_id(record)
    ticket_event = get_event(ticket_eid) or {
        'id': ticket_eid,
        'name': record.get('event_name') or 'Unknown event',
    }

    if ctx['mode'] == 'live':
        if ticket_eid != ctx['target_event_id']:
            phase = event_scan_phase(ticket_event)
            if phase == 'past':
                return 'expired', ticket_event
            return 'wrong_event', ticket_event
        return None, ticket_event

    if ctx['mode'] == 'upcoming':
        upcoming = ctx.get('upcoming_event') or {}
        if ticket_eid == upcoming.get('id'):
            return 'early', ticket_event
        phase = event_scan_phase(ticket_event)
        if phase == 'past':
            return 'expired', ticket_event
        return 'wrong_event', ticket_event
    ...
    if ctx['mode'] == 'past':
        if event_scan_phase(ticket_event) == 'past':
            return 'expired', ticket_event
        return 'wrong_event', ticket_event

    return 'invalid', ticket_event
...
def record_ticket(
    ...
    event = get_sellable_event(event_id) or get_event(event_id) or get_event(LEGACY_EVENT_ID)
    resolved_event_id = event.get('id') if event else (event_id or LEGACY_EVENT_ID)
    ticket_types = get_ticket_types(resolved_event_id)
    ...
    with tickets_lock:
        ...
        ticket = {
            ...
            'event_name': event.get('name') if event else resolved_event_id,
...
def ensure_member_discount_code(member, event_id=None):
    ...
    while discount_code_taken(code, exclude_email=member.get('email')):
        code = generate_discount_code(member.get('email', 'member'))
    origin_event = get_event(event_id) if event_id else None
    with members_lock:
        ...
        for stored in members:
            if stored.get('email', '').lower() == member.get('email', '').lower():
                ...
                if origin_event:
                    stored['discount_code_event_id'] = origin_event.get('id')
                    stored['discount_code_event_name'] = origin_event.get('name')
                save_members(members)
                break
    member['discount_code'] = code
    if origin_event:
        member['discount_code_event_id'] = origin_event.get('id')
        member['discount_code_event_name'] = origin_event.get('name')
    return code
...
def calculate_bulk_total_cents(ticket_type, quantity, ticket_types=None):
    types = ticket_types or TICKET_TYPES
    base = types.get(ticket_type, types['general'])['price_cents']
...
def calculate_total_cents(ticket_type, quantity, apply_member_discount=False, ticket_types=None):
    types = ticket_types or TICKET_TYPES
    base = types.get(ticket_type, types['general'])['price_cents']
    base_total = base * quantity

    if not apply_member_discount or member_discount <= 0:
        return calculate_bulk_total_cents(ticket_type, quantity, ticket_types=types)
...
def calculate_unit_price(ticket_type, quantity, apply_member_discount=False, ticket_types=None):
    if quantity < 1:
        quantity = 1
    return calculate_total_cents(
        ticket_type, quantity, apply_member_discount, ticket_types=ticket_types,
    ) // quantity


def pricing_breakdown(ticket_type, quantity, apply_member_discount=False, event_id=None):
    ticket_types = get_ticket_types(event_id)
    if ticket_type not in ticket_types:
        ticket_type = 'general'
    base = ticket_types[ticket_type]['price_cents']
    base_total_cents = base * quantity
    bulk_only_total = calculate_bulk_total_cents(ticket_type, quantity, ticket_types=ticket_types)
...
def ticket_result_meta(record):
    ...
    types = get_ticket_types(ticket_event_id(record))
    access = record.get('access') or types.get(ticket_type, {}).get('access')
    event = get_event(ticket_event_id(record))
    return {
        'ticket_type': ticket_type,
        'access': access,
        'is_vip': ticket_type == 'vip',
        'event_id': ticket_event_id(record),
        'event_name': record.get('event_name') or (event.get('name') if event else None),
    }
...
def build_wallet_pass(ticket_id, quantity, event_name=None):
    if not wallet_enabled:
        return None

    record = get_ticket_record(ticket_id)
    if record and not event_name:
        event_name = record.get('event_name')
    event_name = event_name or 'The Section'
    verify_url = f"{base_url}/verify/t/{ticket_id}"
    ...
    pass_json = {
        ...
        'description': f'The Section — {event_name}',
...
def get_admission_totals(event_id=None):
    ctx = get_scanner_context()
    filter_event_id = event_id or ctx.get('target_event_id')
    if not filter_event_id and ctx.get('mode') == 'upcoming':
        upcoming = ctx.get('upcoming_event') or {}
        filter_event_id = upcoming.get('id')
    ga = 0
    vip = 0
    for ticket in load_tickets():
        if not ticket.get('scanned_at'):
            continue
        if filter_event_id and ticket_event_id(ticket) != filter_event_id:
            continue
...
def check_ticket(ticket_id):
    ...
    scan_error, ticket_event = validate_ticket_for_scanner(record)
    if scan_error:
        return {
            'status': scan_error,
            ...
            'event_name': meta.get('event_name') or ticket_event.get('name'),
            **{k: v for k, v in meta.items() if k not in ('event_id', 'event_name')},
        }
...
def send_ticket_email(
    ...
    record = get_ticket_record(ticket_id)
    event_id = ticket_event_id(record) if record else None
    type_label = get_ticket_types(event_id).get(ticket_type, TICKET_TYPES['general'])['name']
    event_line = f"Event: {event_name or (record.get('event_name') if record else 'The Section')}\n"
...
def deliver_ticket_email(session_id, customer_email, ticket_id, quantity, ticket_data, ticket_type='general', access=None, event_name=None):
    ...
    def _send():
        result['sent'] = send_ticket_email(
            customer_email, ticket_id, quantity, ticket_data, ticket_type, access, event_name=event_name,
        )
        if result['sent']:
...
@app.route('/')
def home():
    return render_template('home.html', events=list_public_events())


@app.route('/api/events')
def api_events():
    return jsonify({'events': list_public_events()})
...
def member_status():
    ...
    if member:
        ...
        if discount_eligible:
            discount_code = member.get('discount_code') or ensure_member_discount_code(member, event_id=event_id)
    return jsonify({
        'logged_in': bool(member),
        'email': session.get('legacy_member_email'),
        'discount_code': discount_code,
        'discount_code_event_name': member.get('discount_code_event_name') if member else None,
...
def build_checkout_session(quantity, ticket_type, apply_member_discount=False, event_id=None):
    event = get_sellable_event(event_id)
    if not event:
        raise ValueError('Tickets are not available for that event.')
    ...
    breakdown = pricing_breakdown(ticket_type, quantity, apply_member, event_id=event_id)
    ...
    print(f"Creating {ticket_type} session for {quantity} tickets @ {unit_price}c ({event_id})")
    ...
    checkout_kwargs = {
        ...
        'line_items': [{
            'price_data': {
                'currency': 'usd',
                'product_data': {
                    'name': f"The Section - {event.get('name')} - {ticket_meta['name']}",
                    'description': description,
                },
...
def success():
    ...
    try:
        ...
        event_id = metadata.get('event_id') or LEGACY_EVENT_ID
        event = get_event(event_id)
        event_name = metadata.get('event_name') or (event.get('name') if event else event_id)
        ticket_types = get_ticket_types(event_id)
        ...
        if existing_ticket:
            ...
            event_name = existing_ticket.get('event_name', event_name)
            ...
            record_ticket(
                ...
                ticket_type=ticket_type, legacy_discount=legacy_discount, event_id=event_id,
            )
            ...
            if delivery_email:
                ...
                if purchased_member:
                    ...
                    if member_has_past_purchases(purchased_member):
                        ensure_member_discount_code(purchased_member, event_id=event_id)

        ticket_data = build_qr_image(ticket_id)

        email_sent = deliver_ticket_email(
            session_id, delivery_email, ticket_id, quantity, ticket_data,
            ticket_type, access, event_name=event_name,
        )
...
def verify_ticket_native(ticket_id):
    ...
    scanner_ctx = scanner_context_public(get_scanner_context())
    return render_template(
        'verify_result.html',
...
def verify_ticket():
    ...
    if request.method == 'POST':
        ...
        if result['status'] == 'wrong_event':
            return f"❌ Wrong event — ticket is for {result.get('event_name', 'another night')}"
        if result['status'] == 'expired':
            return f"❌ Event ended — {result.get('event_name', 'this ticket')}"
        if result['status'] == 'early':
            return f"❌ Too early — ticket is for {result.get('event_name', 'a future event')}"
        return "Invalid ticket"
...
def download_tickets_csv():
    ...
    writer.writerow([
        'purchased_at', 'ticket_id', 'email', 'quantity', 'event_id', 'event_name',
        'ticket_type', 'access', 'legacy_discount', 'scanned_at', 'email_sent_at', 'verify_url',
    ])
    for ticket in tickets:
        writer.writerow([
            ...
            ticket.get('email', ''),
            ticket.get('quantity', ''),
            ticket.get('event_id', LEGACY_EVENT_ID),
            ticket.get('event_name', ''),
