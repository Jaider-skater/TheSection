...
def calculate_unit_price(ticket_type, quantity):
    ...
    if ticket_type == 'vip' and vip_discount > 0:
        candidates.append(int(base * (1 - vip_discount)))
...
def pricing_breakdown(ticket_type, quantity):
    ...
    vip_price = int(base * (1 - vip_discount)) if ticket_type == 'vip' and vip_discount > 0 else None
    bundle_price = int(base * (1 - bundle_discount)) if bundle_discount_applies(quantity) else None
    return {
        'base_unit_price_cents': base,
        'unit_price_cents': unit_price,
        'vip_discount_applied': ticket_type == 'vip' and vip_price is not None and unit_price == vip_price,
