...
def sync_purchased_tickets_for_member(email):
    if not email:
        return
    normalized_email = email.strip().lower()
    for ticket in load_tickets():
        ticket_email = (ticket.get('email') or '').strip().lower()
        if ticket_email == normalized_email:
            ticket_id = ticket.get('ticket_id')
            if ticket_id:
                add_saved_ticket_for_member(normalized_email, ticket_id)


def auto_save_ticket_for_members(ticket_id, customer_email=None):
    emails = set()
    logged_in = session.get('legacy_member_email')
    if logged_in:
        emails.add(logged_in.strip().lower())
    if customer_email:
        purchase_email = customer_email.strip().lower()
        if get_legacy_member(purchase_email):
            emails.add(purchase_email)
    for email in emails:
        add_saved_ticket_for_member(email, ticket_id)
        member = get_legacy_member(email)
        if member and member_has_past_purchases(member):
            ensure_member_discount_code(member)


def build_saved_ticket_details(member):
    if not member:
        return []
    sync_purchased_tickets_for_member(member['email'])
    member = get_legacy_member(member['email']) or member
    details = []
    for ticket_id in member.get('saved_tickets', []):
        record = get_ticket_record(ticket_id)
        details.append({
            'ticket_id': record.get('ticket_id', ticket_id) if record else ticket_id,
            'purchased_at': record.get('purchased_at') if record else None,
            'quantity': int(record.get('quantity') or 1) if record else 1,
            'ticket_type': record.get('ticket_type', 'general') if record else 'general',
            'view_url': record.get('view_url') if record else None,
        })
    return details
...
def success():
    ...
    try:
        ...
        auto_save_ticket_for_members(ticket_id, customer_email)
...
def legacy_portal():
    member = get_logged_in_member()
    saved_ticket_details = build_saved_ticket_details(member) if member else []
    ...
    if request.method == 'POST':
        ...
        if action == 'forgot_password':
            ...
            if logged_in and logged_in.get('email', '').lower() == email:
                saved_ticket_details = build_saved_ticket_details(logged_in)
                has_past_purchases = member_has_past_purchases(logged_in)
                if has_past_purchases:
                    ensure_member_discount_code(logged_in)
                    logged_in = get_legacy_member(email)
                return render_template(
                    'legacy_portal.html',
                    success=(
    ...
    has_past_purchases = False
    if logged_in_member:
        saved_ticket_details = build_saved_ticket_details(logged_in_member)
        logged_in_member = get_logged_in_member()
        has_past_purchases = member_has_past_purchases(logged_in_member)
        if has_past_purchases:
