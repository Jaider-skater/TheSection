...
def calculate_total_cents(ticket_type, quantity, apply_member_discount=False):
    ...
    if apply_member_discount and member_discount > 0:
        options.append(int(base * (1 - member_discount)) * quantity)
    return min(options)
...
def pricing_breakdown(ticket_type, quantity, apply_member_discount=False):
    ...
    member_total = (
        int(base * (1 - member_discount)) * quantity
        if apply_member_discount and member_discount > 0
        else None
    )

    vip_bundle_applied = vip_bundle_total is not None and total_cents == vip_bundle_total
    bundle_discount_applied = bundle_total is not None and total_cents == bundle_total
    member_discount_applied = member_total is not None and total_cents == member_total
...
def deliver_password_reset_email(customer_email, token):
    result = {'sent': False}

    def _send():
        result['sent'] = send_password_reset_email(customer_email, token)
...
def pricing():
    ...
    if ticket_type not in TICKET_TYPES:
        ticket_type = 'general'
    return jsonify(pricing_breakdown(ticket_type, quantity, member_discount_active()))


def build_checkout_session(quantity, ticket_type):
    if ticket_type not in TICKET_TYPES:
        ticket_type = 'general'
...
def create_checkout_session():
    ...
    try:
        ...
        quantity = max(1, int(data.get('quantity', 1)))
        ticket_type = data.get('ticket_type', 'general')
        checkout_session = build_checkout_session(quantity, ticket_type)
...
def success():
    ...
    try:
        ...
        if existing_ticket:
            ...
            if customer_email:
                purchased_member = get_legacy_member(customer_email)
                if purchased_member:
                    add_saved_ticket_for_member(customer_email, ticket_id)
    ...
    if request.method == 'POST':
        ...
        if action == 'forgot_password':
            ...
            if get_legacy_member(email):
                ...
                if token:
                    deliver_password_reset_email(email, token)
            return render_template(
                'legacy_portal.html',
                **portal_context(
                    success=(
