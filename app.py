...
def portal_context(member=None, saved_ticket_details=None, error=None, next_url='', active_tab='login'):
    ...
    if logged_in:
        ...
        has_past = member_has_past_purchases(logged_in)
        if has_past:
        ...
        for ticket_id in logged_in.get('saved_tickets', []):
            ...
            if record:
                record = dict(record)
                record['view_url'] = record.get('verify_url') or f"{base_url}/verify/t/{ticket_id}"
            saved_ticket_details.append({'ticket_id': ticket_id, 'record': record})
    return {
        'error': error,
        'member': logged_in,
...
def member_portal():
    ...
    if request.method == 'POST':
        ...
        if action == 'set_discount_code' and member:
            ...
            return render_template(
                'legacy_portal.html',
                **portal_context(member=member, error=error, next_url=next_url),
            )
