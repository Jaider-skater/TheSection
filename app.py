...
# Stripe — set STRIPE_SECRET_KEY on Render (sk_live_... for production, sk_test_... for sandbox).
# Do not hardcode keys; a baked-in sk_test key always forces sandbox even if live env vars exist.
stripe.api_key = clean_env_value(
...
if stripe.api_key.startswith('sk_live_'):
    ...
    print('Stripe mode: TEST (sandbox)')
elif stripe.api_key:
    print('Stripe mode: unknown key prefix (check STRIPE_SECRET_KEY)')
else:
    print('WARNING: STRIPE_SECRET_KEY is not set — checkout will fail until it is configured')
...
def verify_scanner_credentials(email, password):
    if not verify_auth_configured():
        return False
    email_ok = secure_equal(
        (email or '').strip().lower(),
        verify_login_email,
    )
    # Strip accidental leading/trailing spaces from the form field (mobile paste).
    password_ok = secure_equal((password or '').strip(), verify_login_password)
...
def verify_logout():
    ...
    # Scanner access can come from the member portal admin account; clear that too
    # or verify_login immediately redirects back into /verify.
