...
let pageEvents = Array.isArray(window.PAGE_EVENTS) ? window.PAGE_EVENTS : [];
let memberStatus = {
    logged_in: false,
    bundle_min: 4,
...
function selectedEvent() {
    return pageEvents.find((event) => event.id === selectedEventId) || pageEvents[0] || null;
}

function eventQuery() {
    return selectedEventId ? `&event_id=${encodeURIComponent(selectedEventId)}` : '';
}

function updateModalEventLabel() {
    const label = document.getElementById('modal-event-name');
    const event = selectedEvent();
    if (label) {
        label.textContent = event ? event.name : '';
    }
}

async function loadMemberStatus() {
    try {
        const response = await fetch(`/api/member-status?${eventQuery().replace(/^&/, '')}`);
...
function updateMemberBanner() {
    ...
    const discountOrigin = document.getElementById('member-discount-origin');
    const discountBtn = document.getElementById('member-discount-code-btn');
    if (memberStatus.logged_in) {
        ...
        if (discountOrigin) {
            if (memberStatus.discount_code_event_name) {
                discountOrigin.textContent = `Earned at ${memberStatus.discount_code_event_name} · works on every event`;
                discountOrigin.classList.remove('hidden');
            } else {
                discountOrigin.classList.add('hidden');
                discountOrigin.textContent = '';
            }
        }
        updateDiscountCodeButton();
        if (discountBtn) {
...
async function showTicketsModal(options = {}) {
    if (options.eventId) {
        selectedEventId = options.eventId;
    } else if (!selectedEventId && pageEvents.length) {
        selectedEventId = pageEvents[0].id;
    }
    updateModalEventLabel();
...
document.querySelectorAll('.event-ticket-link, a[href="#tickets"]').forEach(link => {
    link.addEventListener('click', function(e) {
        e.preventDefault();
        showTicketsModal({ eventId: link.dataset.eventId || null });
    });
});
